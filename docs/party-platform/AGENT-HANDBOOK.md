...
For 3D or continuous-input work, apply [3D-READINESS.md](3D-READINESS.md) before committing to an engine, timing model, or ten-camera scene. Kart Party’s engine lives in `packages/games/kart-party/src/engine`, beside its shared adapter in `packages/games/kart-party/src`. All nine games use the same room and `/ws` connection. The `/kart-party/` landing route redirects into PartyPlay. The untouched original reference remains at `/Users/joshthemenace/Documents/Codex/2026-09-06/i-want-you-to-explore-building/outputs/kart-party`; inspect its actual source when needed. Use the integrated module and its tests for current racing behavior.
