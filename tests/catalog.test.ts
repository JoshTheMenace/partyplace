...
test('library exposes all eleven games and truthful solo, shared-screen and development filters', () => {
  assert.equal(catalog.length, 11);
  assert.deepEqual(filterCatalog(catalog, '', 'solo').map(g => g.id), ['kart-party','island-settlers','blockwild','kitchen-rush']);
  assert.deepEqual(filterCatalog(catalog, '', 'no-tv').map(g => g.id), ['kart-party','island-settlers','blockwild','kitchen-rush']);
...
test('search combines words with the selected filter and handles empty results', () => {
  ...
  assert.equal(filterCatalog(catalog, '   ', 'all').length, 11);
});
