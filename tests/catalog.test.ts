...
test('library exposes all twelve games and truthful solo, shared-screen and development filters', () => {
  assert.equal(catalog.length, 12);
  assert.deepEqual(filterCatalog(catalog, '', 'solo').map(g => g.id), ['kart-party','starship-scramble','blockwild','kitchen-rush']);
  assert.deepEqual(filterCatalog(catalog, '', 'no-tv').map(g => g.id), ['kart-party','starship-scramble','blockwild','kitchen-rush']);
  assert.equal(filterCatalog(catalog, '', 'shared-screen').length, 8);
  ...
  assert.equal(filterCatalog(catalog, '', 'party').length, 8);
  assert.deepEqual(filterCatalog(catalog, '', 'co-op').map(g=>g.id).sort(), ['blockwild','kitchen-rush','starship-scramble']);
});
