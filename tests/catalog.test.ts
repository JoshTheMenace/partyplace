...
test('library exposes all twelve games and truthful solo, shared-screen and development filters', () => {
  ...
  assert.deepEqual(filterCatalog(catalog, '', 'solo').map(g => g.id), ['kart-party','starship-scramble','island-settlers','blockwild','kitchen-rush']);
  assert.deepEqual(filterCatalog(catalog, '', 'no-tv').map(g => g.id), ['kart-party','starship-scramble','island-settlers','blockwild','kitchen-rush']);
