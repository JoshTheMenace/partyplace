...
export function parsePublicOrigin(value?: string): string | undefined {
  ...
  if (!['https:', 'http:'].includes(url.protocol) || url.username || url.password || url.pathname !== '/' || url.search || url.hash) throw Error('PUBLIC_ORIGIN must be an HTTP(S) origin without a path, credentials, query, or fragment.');
  return url.origin;
}
