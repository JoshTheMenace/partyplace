...
const details: Record<string, Pick<CatalogGame, 'category' | 'status' | 'playTime' | 'searchTerms'>> = {
  'ichi': { category: 'Party', status: 'ready', playTime: '10–25 min', searchTerms: ['cards', 'strategy', 'uno', 'stacking', 'ichi'] },
  'night-job': { category: 'Co-op', status: 'ready', playTime: 'Quick heists · replayable', searchTerms: ['heist', 'stealth', 'thieves', 'cooperative', 'pixel art', 'specialists'] },
...
const descriptions: Record<string, string> = {
  'night-job': 'Three original pixel-art heists. Eight specialists. Get the goods and your whole crew out.',
...
export const kartParty: CatalogGame = { id:'kart-party',title:'Kart Party',description:'Five toybox courses, finishing on Rainbow Road, and eight racers. Drift for mini-turbos, grab items, and race friends or CPUs.',category:'Racing',status:'ready',players:{min:1,max:10},supportsSolo:true,requiresSharedDisplay:false,cooperative:false,controls:['Keyboard','Touch','Phone controllers'],playTime:'5–15 min',launch:'room',searchTerms:['racing','driving','solo','cpu','3d'],tags:['racing','driving','solo','CPU rivals'],creator:{name:'PartyPlay'},detail:'Race solo against CPU drivers or bring up to ten friends. Play on this screen with a keyboard, touch or gamepad, or use phones as controllers. Up to four racers share the TV in split screen; bigger groups race on their own phones.',capabilities:['browser-play','solo','keyboard','touch','phone-controllers'] };
export function gameCatalog(manifests: readonly GameManifest[]): CatalogGame[] {
  return [...(manifests.some(m=>m.id==='kart-party')?[kartParty]:[]),...manifests.filter(m=>!['scene-lab','kart-party'].includes(m.id)).map(m=>({id:m.id,title:m.title,description:descriptions[m.id]??m.description,players:m.players,supportsSolo:m.supportsSolo,requiresSharedDisplay:!m.supportsSolo && m.modes.every(mode=>mode==='shared-display'),cooperative:['night-job','blockwild','kitchen-rush','starship-scramble','phonedig'].includes(m.id),controls:m.id==='night-job'?['Touch','Phone controllers']:m.id==='hotpot'?['Mouse','Touch','Phone controllers']:m.supportsSolo?['Keyboard','Touch','Phone controllers']:['Phone controllers'],launch:'room' as const,creator:{name:['phonedig','hotpot'].includes(m.id)?'ahcomputing':'PartyPlay'},detail:m.id==='night-job'?'Choose from eight specialists across three original pixel-art heists. Use 1–4 phones as touch controllers and a TV or browser as the shared map display.':descriptions[m.id]??m.description,capabilities:['browser-play','shared-display','phone-controllers',...(m.id==='night-job'?['touch']:[]),...(m.supportsSolo?['solo','touch',...(m.id==='hotpot'?[]:['keyboard'])]:[]),...(m.sessionControls?.includes('save')?['server-autosave']:[]),...(m.id==='kitchen-rush'?['host-browser-progress']:[])] as CatalogCapability[],...(details[m.id]??{category:'Party' as const,status:'in-progress' as const,playTime:'Varies',searchTerms:[]})}))];
}
