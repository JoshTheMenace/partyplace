...
export function gameCatalog(manifests: readonly GameManifest[]): CatalogGame[] {
  return [...(manifests.some(m=>m.id==='kart-party')?[kartParty]:[]),...manifests.filter(m=>!['scene-lab','kart-party'].includes(m.id)).map(m=>({id:m.id,title:m.title,description:descriptions[m.id]??m.description,players:m.players,supportsSolo:m.supportsSolo,requiresSharedDisplay:!m.supportsSolo && m.modes.every(mode=>mode==='shared-display'),cooperative:['blockwild','kitchen-rush'].includes(m.id),controls:m.supportsSolo?['Keyboard','Touch','Phone controllers']:['Phone controllers'],launch:'room' as const,creator:{name:'PartyPlay'},detail:descriptions[m.id]??m.description,capabilities:['browser-play','shared-display','phone-controllers',...(m.supportsSolo?['solo','keyboard','touch']:[]),...(m.sessionControls?.includes('save')?['server-autosave']:[]),...(m.id==='kitchen-rush'?['host-browser-progress']:[])] as CatalogCapability[],...(details[m.id]??{category:'Party' as const,status:'in-progress' as const,playTime:'Varies',searchTerms:[]})}))];
}
