import { PhoneScreenButton, usePhoneViewport } from './phone-screen';
...
import { ArcadeButton, Eyebrow, Modal, Panel, StatusNotice, TextInput, usePhoneOrientation } from '../../../packages/party-ui/src/index';
...
function App() {
  ...
  const [hostKind, setHostKind] = useState<'hosted' | 'local'>('hosted');
  const [stopped, setStopped] = useState<'idle' | 'stopping' | 'stopped' | 'failed'>('idle');
  ...
  return <main className={`kp-shell ${inGame ? 'kp-shell-playing' : ''} ${phone ? 'kp-shell-phone' : ''} ${phone && module?.immersivePhone ? 'kp-shell-immersive' : ''}`}><header className="kp-header">{room ? <div className="kp-header-brand"><span className="kp-brand-mark" aria-hidden="true">✦</span><span className="kp-display">{inGame && game ? game.title : 'PartyPlay'}</span></div> : <Link route={{ view: 'home' }} className="kp-header-brand kp-header-brand-link" aria-label="PartyPlay home" onClick={event => { if (deepJoin) { event.preventDefault(); location.assign('/'); } }}><span className="kp-brand-mark" aria-hidden="true">✦</span><span className="kp-display">PartyPlay</span></Link>}<div className="kp-header-actions">{room && <span className="kp-session-pill">{phone ? player?.name ?? name : isHost ? '' : 'Watching'} <b className="kp-numeral">{room.code}</b></span>}{handheldPlayer && !!module?.immersivePhone && <PhoneScreenButton/>}<ArcadeButton size="sm" tone="ghost" onClick={() => { if (muted) void music.enable().then(() => setMuted(false)).catch(() => setLoadError('Sound could not start. Tap Sound on again.')); else { music.setMuted(true); setMuted(true); } }}>{muted ? 'Sound off' : 'Sound on'}</ArcadeButton>{isHost && inGame && <ArcadeButton size="sm" tone="ghost" onClick={() => setMenuOpen(true)}>Room menu</ArcadeButton>}</div></header>
    {room && identity && module?.AudioView && <module.AudioView key={room.gameId} phase={room.phase} roundId={room.roundId} publicView={snapshot?.roundId === room.roundId && ['playing', 'results'].includes(room.phase) ? snapshot?.publicView ?? null : null} connected={connected} viewRole={role} isHost={isHost} serverNowMs={session.serverNowMs}/>}
    ...
    {!room || !identity ? deepJoin ? <Panel className="kp-quick-join"><Eyebrow>Your phone is your controller</Eyebrow><h1>You're invited.</h1><p>Enter your name. The fun is on the big screen.</p>{joinForm}<button className="kp-text-button" onClick={() => setHelpOpen(true)}>Trouble joining?</button></Panel> : <><LibraryNavigation view={route.view}/>{route.view === 'game' ? <GamePage id={route.id} game={routeGame} loading={curated.loading} all={catalog} manifest={manifestFor(route.id)} onPlay={playFromLibrary} canPlay={canJoin && !pendingGame} onJoin={() => { setJoinOpen(true); }} origin={address || location.origin}/> : <GameDashboard key={route.view} view={route.view} games={catalog} sources={curated.sources} loading={curated.loading} onPlay={playFromLibrary} canPlay={canJoin && !pendingGame} actions={<><ArcadeButton size="sm" tone="ghost" onClick={() => setJoinOpen(true)}>Join with a code</ArcadeButton>{route.view === 'discover' && <button className="kp-text-button" onClick={() => void curated.refresh()}>Refresh sources</button>}</>}/>}{curated.error && <StatusNotice tone="error">{curated.error} <button className="kp-text-button" onClick={() => void curated.refresh()}>Retry</button></StatusNotice>}{pendingGame && <StatusNotice>Opening your game room…</StatusNotice>}<button className="kp-text-button" onClick={() => setHelpOpen(true)}>Setup & connection help</button></> : <>
      {!inGame ? <div className={`kp-room-layout ${phone ? 'kp-room-layout-phone' : ''}`}>{roomRail}<section className="kp-room-main">
        ...
        {game && room.phase === 'lobby' && <GameBoundary key={`${game.id}:lobby`} onFailure={renderFailure}>{!(phone && module?.LobbyView) && <div className="kp-lobby-title">{game.id === 'kart-party' ? <KartArt/> : <GameArt id={game.id}/>}<div><Eyebrow>{game.players.min}–{game.players.max} players · {game.orientation.controller} phones</Eyebrow><h1>{game.title}</h1><p>{gameLooks[game.id]?.verb ?? game.description}</p></div></div>}{loadingNotice}{!module && !loadError && <StatusNotice>Loading {game.title}…</StatusNotice>}{module && <>{(isHost || player && !module.LobbyView) && <div className="kp-lobby-launch">{player && !isHost && !module.LobbyView && <><p>{player.ready ? 'You’re ready. Watch the shared screen.' : `Ready, ${player.name}?`}</p><ArcadeButton size="xl" tone={player.ready ? 'ghost' : 'lime'} disabled={!connected} onClick={() => session.send('room.ready', { ready: !player.ready, lobbyId: room.lobbyId })}>{player.ready ? 'Not ready' : 'Ready to play'}</ArcadeButton></>}{isHost && <>{game.supportsSolo && <ArcadeButton size="sm" tone="ghost" disabled={!connected} onClick={() => session.send('room.play', { play: !player })}>{player ? 'Watch only' : 'Play on this device'}</ArcadeButton>}<div><Eyebrow>{room.players.filter(item => item.ready).length}/{room.players.length} ready</Eyebrow><p>{readyReason || 'Everyone is ready. Let’s play.'}</p></div><ArcadeButton size="lg" disabled={!!readyReason || !connected} onClick={() => session.send('round.start')}>Start game</ArcadeButton><ArcadeButton size="sm" tone="ghost" onClick={() => setSettingsOpen(true)}>Settings</ArcadeButton></>}</div>}<>{module.LobbyView && <module.LobbyView key={room.lobbyId} roomId={room.id} lobbyId={room.lobbyId!} players={room.players} playerId={identity.playerId} isHost={isHost} settings={room.settings} connected={connected} error={state.error} onChoice={payload => { session.send('lobby.choice', { lobbyId: room.lobbyId, payload }); }} onReady={ready => { session.send('room.ready', { ready, lobbyId: room.lobbyId }); }}/>}</>{!(phone && module.LobbyView) && (module.LobbyView ? <details className="kp-player-details"><summary>How to play</summary><module.InstructionsView role={role}/></details> : <Panel className="kp-instructions-panel"><module.InstructionsView role={role}/></Panel>)}{isHost && game.sessionControls?.includes('save') && <RecoveryControls key={game.id} session={session} gameId={game.id} connected={connected} canRestore={!readyReason}/> }{phone && !module.LobbyView && <details className="kp-player-details"><summary>Who's here · {room.players.length} players</summary>{roster}</details>}</>}</GameBoundary>}
    ...
    {helpOpen && <Modal title="Get everyone connected" onClose={() => setHelpOpen(false)}><p>Open PartyPlay on a laptop or TV everyone can see. Pick a game, then have players scan the room’s QR code or enter its code on their phones.</p>{addresses.length > 1 && <label>Choose network address<select value={address} onChange={event => setAddress(event.target.value)}>{addresses.map(url => <option key={url}>{url}</option>)}</select></label>}{joinUrl && <p><a href={joinUrl}>{joinUrl}</a></p>}<p>Use the same site address on every device. A local network address needs the same Wi-Fi; a public link works over the internet. Remote players still need to see the shared display, for example through a screen-sharing call.</p><p className="kp-muted">Kart Party, Blockwild and Kitchen Rush can also be played alone on this device.</p>{hostKind === 'local' && <section className="kp-local-host" aria-label="Local host">{['localhost', '127.0.0.1', '[::1]'].includes(location.hostname) ? <><Eyebrow>Running on this computer</Eyebrow><p>PartyPlay is hosted by this computer. World saves stay in its PartyPlay data folder, not on the internet.</p></> : <><Eyebrow>Hosted on a computer on this network</Eyebrow><p>This room runs on the computer that opened PartyPlay.app, not on the internet. World saves stay in that computer’s PartyPlay data folder.</p></>}{['localhost', '127.0.0.1', '[::1]'].includes(location.hostname) ? stopped === 'stopped' ? <StatusNotice tone="success">PartyPlay stopped. Close this window; open the app again to host another night.</StatusNotice> : <ArcadeButton size="sm" tone="coral" disabled={stopped === 'stopping'} onClick={() => { setStopped('stopping'); fetch('/api/local/stop', { method: 'POST' }).then(response => setStopped(response.ok ? 'stopped' : 'failed')).catch(() => setStopped('failed')); }}>{stopped === 'stopping' ? 'Stopping…' : 'Stop hosting on this computer'}</ArcadeButton> : <p className="kp-muted">Only the hosting computer can stop PartyPlay: from its browser, or with Stop PartyPlay in the downloaded folder.</p>}{stopped === 'failed' && <StatusNotice tone="error">PartyPlay could not be stopped from here. Use Stop PartyPlay in the downloaded folder.</StatusNotice>}</section>}</Modal>}
